...
// Order: NFL, MLB, NHL, NCAAH, NBA, NCAAF, NCAAB
const char* league_names[] = {"NFL","MLB","NHL","NCAAH","NBA","NCAAF","NCAAB"};
const char* sport_names[]  = {"football","baseball","hockey","hockey",
                              "basketball","football","basketball"};
const char* espn_slugs[]   = {"nfl","mlb","nhl","mens-college-hockey",
                              "nba","college-football",
                              "mens-college-basketball"};
const char* logo_folders[] = {"team0_logos","team1_logos","team2_logos",
                              "team6_logos","team3_logos","team4_logos",
                              "team5_logos"};

// Alert messages per league (for score changes)
const char* alert_messages[] = {
  "TOUCHDOWN!", "RUN SCORED!", "GOAL!", "GOAL!",
  NULL, "TOUCHDOWN!", NULL
};
...
const char** my_teams[] = {
  my_nfl, my_mlb, my_nhl, my_ncaah, my_nba, my_ncaaf, my_ncaab
};

// ============================================================
//  GAME DATA
// ============================================================
// 96 slots: enough for a full March Madness day (32 NCAAB games) plus
// active NBA/NHL/MLB slates without silent drop. The golden baseline had
// 64, which was tight on busy tournament days.
#define MAX_GAMES 96
...
// Note: showScoreAlert calls drawBMP 9 times (4+4+1 flash frames x 2 logos).
// This is redundant LittleFS I/O but doesn't cause visual flicker — Protomatter
// is double-buffered so all drawing goes to the back buffer before show().
// The 400ms delay() calls between frames dominate timing, making the file reads
// negligible. Logo caching (ps_malloc'd uint16_t pixel arrays) could eliminate
// this if I/O ever becomes a bottleneck.
